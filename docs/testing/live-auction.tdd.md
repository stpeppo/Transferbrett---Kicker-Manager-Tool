...
|---|---|---|---|
| Auction domain transitions | `tests/auction_logic.test.js` | `node --test tests/auction_logic.test.js` failed with `Cannot find module '../auction_logic'` before production logic existed; later regression tests covered leave handling and Firebase-omitted null bid fields. | 21 domain tests pass after `auction_logic.js`, its authorization guards, active leave handling, and Firebase round-trip normalization were implemented. |
| Build and UI integration | `tests/auction_build_integration.test.js` | Three assertions failed because the panel, controls, build placeholder, and inlined logic did not yet exist. | All thirteen integration checks pass after rebuilding `transferbrett.html`. |
...
- Tests: bundled Node `--test --experimental-test-coverage tests/auction_logic.test.js tests/auction_build_integration.test.js` — 34/34 PASS at the recorded full-suite checkpoint.
- Domain coverage at that checkpoint: 98.18% lines, 84.91% branches, 96.77% functions.
